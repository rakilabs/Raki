...
fn pin_only_booster() -> DefaultSignalBooster {
    DefaultSignalBooster::new(MixerConfig::new(10_000.0, 0.25, 0.0, 2.0).unwrap())
}

fn salience_only_booster() -> DefaultSignalBooster {
    DefaultSignalBooster::new(MixerConfig::new(10_000.0, 0.0, 0.15, 2.0).unwrap())
}
...
pub async fn ablate_signals(
    ...
    .await?;
    let baseline_success = baseline.iter().filter(|r| r.success_at_3_boosted).count();
    let total = baseline.len();

    let configs: [(&'static str, DefaultSignalBooster); 4] = [
        ("all", all_signals_booster()),
        ("recency", recency_only_booster()),
        ("pin", pin_only_booster()),
        ("salience", salience_only_booster()),
    ];
    ...
    for (name, booster) in configs {
        let run = measure_lift(
            keyword,
            vectors,
            embedder,
            signal_source,
        ...
        .await?;
        let success = run.iter().filter(|r| r.success_at_3_boosted).count();
